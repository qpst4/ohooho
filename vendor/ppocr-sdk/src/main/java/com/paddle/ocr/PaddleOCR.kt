...
class PaddleOCR private constructor(
    ...
    companion object {
        ...
        suspend fun createFromFiles(
            context: Context,
            config: PaddleOCRConfig,
            engineConfig: EngineConfig,
            detModelFilePath: String,
            recModelFilePath: String,
            recConfigFilePath: String,
        ): PaddleOCR {
            val appContext = context.applicationContext
            return withContext(Dispatchers.IO) {
                val engine = OCREngine(
                    ...
                    config,
                    engineConfig,
                    detModelFile = detModelFilePath,
                    recModelFile = recModelFilePath,
                    recConfigFile = recConfigFilePath,
                )
                PaddleOCR(engine)
            }
        }
