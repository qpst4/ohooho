...
class ORTSessionManager(
    ...
    fun loadModels(detAssetPath: String, recAssetPath: String) {
        loadModelsInternal(
            detLoader = { readModelAsset(detAssetPath) },
            recLoader = { readModelAsset(recAssetPath) },
        )
    }

    fun loadModelsFromFiles(detFilePath: String, recFilePath: String) {
        loadModelsInternal(
            detLoader = { readModelFile(detFilePath) },
            recLoader = { readModelFile(recFilePath) },
        )
    }
    ...
    private fun loadModelsInternal(
        ...
        try {
            ...
            val detBytes = detLoader()
            val recBytes = recLoader()
            try {
