...
data class ModelConfig(
    ...
    companion object {
        fun parse(context: Context, assetPath: String): ModelConfig =
            parseContent(readAssetText(context, assetPath), assetPath)

        fun parseFile(filePath: String): ModelConfig =
            parseContent(readFileText(filePath), filePath)
        ...
        private fun readFileText(filePath: String): String {
            return try {
                java.io.File(filePath).bufferedReader().use { it.readText() }
            } catch (t: Throwable) {
                throw OCRError.ConfigParseFailed(filePath, t)
            }
        }

        private fun parseContent(content: String, sourcePath: String): ModelConfig {
