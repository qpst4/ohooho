...
object RegionalScreenshotOcr {
    ...
    private const val OCR_MAX_LONG_EDGE_PX = 960
    ...
    suspend fun recognizeBitmapPublic(
        ...
        val ocrBitmap = downscaleForOcr(bitmap)
        val recycleScaled = ocrBitmap !== bitmap
        return try {
            service.recognizeBitmap(modelId, ocrBitmap)
    ...
    private fun downscaleForOcr(bitmap: Bitmap): Bitmap {
        ...
        if (longEdge <= OCR_MAX_LONG_EDGE_PX) return bitmap
        val scale = OCR_MAX_LONG_EDGE_PX.toFloat() / longEdge
